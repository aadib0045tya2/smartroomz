...
const blankProperty = {
  ...
  monthlyPrice: 800, deposit: 100, applicationFee: 50, roomType: 'Private room', availability: 'Available now',
...
function PropertyEditor({ property, onClose, onSaved }) {
  ...
  return <div className="admin-editor-backdrop"><form className="admin-editor" onSubmit={save}><header><div><p className="eyebrow">Listing editor</p><h2>{property.title || 'New room'}</h2></div><button type="button" onClick={onClose}>×</button></header><div className="editor-body">
    <div className="form-grid"><Field label="Listing ID (slug)" value={form.id} onChange={(v) => update('id', v)} disabled={Boolean(property.title)} /><Field label="Title" value={form.title} onChange={(v) => update('title', v)} /><Field label="Area" value={form.area} onChange={(v) => update('area', v)} /><Field label="City" value={form.city} onChange={(v) => update('city', v)} /><Field label="State" value={form.state} onChange={(v) => update('state', v)} /><Field label="ZIP" value={form.zip} onChange={(v) => update('zip', v)} /><Field type="number" label="Weekly price" value={form.weeklyPrice} onChange={(v) => update('weeklyPrice', v)} /><Field type="number" label="Bi-weekly price" value={form.biweeklyPrice} onChange={(v) => update('biweeklyPrice', v)} /><Field type="number" label="Monthly price" value={form.monthlyPrice} onChange={(v) => update('monthlyPrice', v)} /><Field type="date" label="Earliest move-in" value={form.earliestMoveInDate || ''} onChange={(v) => update('earliestMoveInDate', v)} /></div>
