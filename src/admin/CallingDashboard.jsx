...
function formatAppointment(call) {
  const startTime = call.structured_data?.appointmentStartTime
  if (startTime) {
  ...
  if (call.appointment_date_text || call.appointment_time_text) {
    return [call.appointment_date_text, call.appointment_time_text].filter(Boolean).join(' · ')
  }
  return call.appointment_booked ? 'Date not captured' : '—'
}
...
export default function CallingDashboard({ calls, session, onReload, onMessage }) {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      recordings: filtered.filter((call) => call.recording_url).length,
  ...
  return <div className="calling-dashboard">
    ...
    <div className="admin-table-wrap call-table"><table className="admin-table"><thead><tr><th>Caller</th><th>Call date</th><th>Duration</th><th>Outcome</th><th>Booked</th><th>Appointment date & time</th><th>Live transferred</th><th>Recording</th><th>Cost</th><th /></tr></thead><tbody>{filtered.map((call) => <tr key={call.id}><td><strong>{call.customer_name || shortPhone(call.caller_number)}</strong><small>{call.customer_name ? shortPhone(call.caller_number) : call.is_test ? 'Test caller' : 'Client'}</small></td><td>{new Date(call.started_at).toLocaleString()}</td><td>{formatDuration(call.duration_seconds)}</td><td><span className="call-outcome"><i style={{ background: COLORS[call.reviewed_category] }} />{OUTCOMES[call.reviewed_category] || 'Other / unclear'}</span></td><td>{call.appointment_booked ? 'Yes' : '—'}</td><td>{formatAppointment(call)}</td><td>{call.transferred_to_dossy ? 'Yes' : '—'}</td><td>{call.recording_url ? <button className="table-button recording-button" onClick={() => setSelected(call)}><Headphones size={13} /> Listen</button> : '—'}</td><td>${Number(call.cost || 0).toFixed(2)}</td><td><button className="table-button" onClick={() => setSelected(call)}>Details</button></td></tr>)}</tbody></table>{!filtered.length && <p className="call-empty">No calls match these filters.</p>}</div>
