...
export default function RequestCallModal({ property, moveInDate = '', properties, onSubmit, onClose }) {
  ...
  const submit = (event) => {
    ...
    // TODO: Replace local submission with GoHighLevel contact/opportunity creation.
    onSubmit({ id: `call-${Date.now()}`, ...form, createdAt: new Date().toISOString() })
    setSent(true)
  }
  return <Modal onClose={onClose} className="form-modal" label="Request a call">{sent ? <div className="success-state"><CheckCircle2 size={50} /><p className="eyebrow">Request saved</p><h2>We’ve got your details.</h2><p>Your request is stored on this device for now. Future versions will send it to the Smart Roomz team.</p><button className="primary-button" onClick={onClose}>Done</button></div> : <><p className="eyebrow">Talk to a housing specialist</p><h2>Request a call</h2><p className="form-intro">Tell us what you’re looking for and the best way to reach you.</p><form onSubmit={submit} className="form-stack"><Field label="Name" value={form.name} error={errors.name} onChange={(v) => update('name', v)} /><Field type="tel" label="Phone" value={form.phone} error={errors.phone} onChange={(v) => update('phone', v)} /><Field type="email" label="Email" value={form.email} error={errors.email} onChange={(v) => update('email', v)} /><label className="field"><span>Property</span><select value={form.propertyId} onChange={(e) => update('propertyId', e.target.value)}>{properties.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><Field type="date" label="Preferred move-in date" value={form.moveInDate} error={errors.moveInDate} onChange={(v) => update('moveInDate', v)} /><label className="field"><span>Optional message</span><textarea value={form.message} onChange={(e) => update('message', e.target.value)} placeholder="Anything we should know?" /></label><button className="primary-button wide" type="submit">Request my call</button></form></>}</Modal>
}
