...
export default function ApplicationModal({ context, onSubmit, onClose }) {
  ...
  const submit = async () => {
    ...
    try { await onSubmit({ id: `app-${Date.now()}`, propertyId: context.property.id, property: context.property.title, applicant: `${form.firstName} ${form.lastName}`, ...form, estimatedAmount: pricing.total, status: 'Submitted', submittedAt: new Date().toISOString() }); setSubmitted(true) }
    catch (error) { setSubmitError(error.message || 'Could not submit your application. Please try again.') }
    finally { setSaving(false) }
  }
  if (submitted) return <Modal onClose={onClose} className="form-modal" label="Application submitted"><div className="success-state"><CheckCircle2 size={50} /><p className="eyebrow">Application received</p><h2>You’re one step closer.</h2><p>Your application was sent securely to the Smart Roomz team. A housing specialist will contact you next.</p><button className="primary-button" onClick={onClose}>Done</button></div></Modal>
  return (
    <Modal onClose={onClose} className="form-modal" label="Room application">
      ...
      {submitError && <p className="form-error">{submitError}</p>}<div className="modal-nav">{step > 1 ? <button className="secondary-button" onClick={() => setStep(step - 1)}>Back</button> : <span />}{step < 3 ? <button className="primary-button" onClick={next}>Continue</button> : <button className="primary-button" disabled={saving} onClick={submit}>{saving ? 'Submitting…' : 'Submit application'}</button>}</div>
    </Modal>
  )
}
